...
- 首个有效输出超时可直接 502。
- 首个有效输出超时可重试并恢复。
- first-progress timer 回调被延迟时，每个 lifecycle/metadata chunk 仍先按绝对墙钟复核，首个过期 chunk 立即返回 502。
- 总 deadline 跨内部重试不重置。
- 总超时耗尽后返回 502。
- 总超时触发后不得再创建新 attempt。
- 已透传后总超时只断连，不重试、不追加第二轮输出。
- 非流式 body stall 可被总超时取消。
- 同一流式 chunk 同时形成超大 SSE 候选和迟到 first-progress 时，检查上限先于 timeout 收口。
- body/chunk 在 deadline 前到达但 JSON/SSE/结构同步处理跨线时，客户端仍收到 total-timeout 502，不能先写 200。
- reader 在 deadline 后异常且 timer 回调尚未执行时，按绝对墙钟归类 timeout，不得降级成普通 stream termination。
- retry 的 header/request 同步准备必须发生在最终闸门前；deadline 后不增加共享预算、代理总数或真实上游请求。
- 首次 total 从第一次真实 fetch 建立，current first-progress 与 `upstream_fetch_started_at_ms` 使用同一锚点；未派发候选不得增加 inspected 或持久化 attempt。
- 非流式 JSON 与 EOF-only SSE 的 first-progress 窗口在语义/结构处理后关闭；流式 header copy 后必须紧邻 `writeHead` 再检查 total。
- `finalizeModelInsights()` 每个 attempt model context 只提交一次，timeout 二次收口不得重复增加模型和 consistency 计数。
...
- 误标 SSE 的首个事件即为超大 `response.completed`，不能依赖更早的普通输出把 parser 预先切到 confirmed 状态。
- UTF-8 BOM 的三个字节跨网络 chunk 时仍不算首 progress，且不改变原始下游字节。
- PowerShell canonical 配置比较保留标量数组的值和顺序，只忽略对象键顺序。
- 非流式 observe-only 的客户端首写/forwarding 字段在实际透传后落盘。
