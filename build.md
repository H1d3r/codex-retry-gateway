...
- timer 回调不是 deadline 的唯一真源；非流式 body、流式 chunk、EOF 与 retry 派发都必须在完成/透传前按绝对时间复核，延迟 timer 故障注入下仍返回对应 first-progress/total 502。
...
- SSE framing 必须覆盖字段名与 JSON 跨 chunk、fallback 后尾随候选、独立/同块 UTF-8 BOM、误标候选超限、LF/CR/CRLF 混合空行和 EOF 纯 CR 终态；reasoning 保护下的超大事件在未写响应时返回专用 502，在已写响应时 fail-closed 断连；EOF 才命中的 disconnect 规则也必须实际断连。
